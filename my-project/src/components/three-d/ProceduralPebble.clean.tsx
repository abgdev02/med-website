...
export function ProceduralPebble({ 
    ...
    if (!isStatic) {
      ...
      for (let i = 0; i < pos.count; i++) {
        ...
        if (distance > 25) {
          ...
          y * flattenFactor + totalNoise * bump * 0.02 + 0.8, // Shifted upward by 0.15 units
  ...
  useEffect(() => {
    ...
    const rotationSpeed = getAnimationConfig(
      distance > 25 ? 0.15 : 0.25, // Normal speed
      distance > 25 ? 0.03 : 0.05  // Reduced motion speed (20% of normal)
    )
    const updateInterval = prefersReducedMotion 
      ? (distance > 25 ? 64 : 32) // Slower updates for reduced motion
      : (distance > 25 ? 32 : 16) // Normal updates
  ...
  }, [animate, distance, prefersReducedMotion, getAnimationConfig])
